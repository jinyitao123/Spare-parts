...
export const inventory: InventoryItem[] = [
  ...
  { partCode: 'BJ-0421', warehouse: '二级库-工段A', stock: 2, safetyStock: 2, amount: 400, lastChange: '2026-03-19', monthlyUsage: 6, status: 'critical' },
  { partCode: 'BJ-0421', warehouse: '二级库-工段B', stock: 0, safetyStock: 2, amount: 0, lastChange: '2026-03-15', monthlyUsage: 4, status: 'shortage' },
  { partCode: 'BJ-0421', warehouse: '二级库-工段C', stock: 3, safetyStock: 2, amount: 600, lastChange: '2026-03-18', monthlyUsage: 3, status: 'sufficient' },
  ...
  { partCode: 'BJ-0535', warehouse: '二级库-工段A', stock: 5, safetyStock: 2, amount: 40000, lastChange: '2026-03-18', monthlyUsage: 1, status: 'sufficient' },
  ...
  { partCode: 'BJ-0156', warehouse: '二级库-工段A', stock: 8, safetyStock: 2, amount: 1600, lastChange: '2026-03-19', monthlyUsage: 4, status: 'sufficient' },
...
export const warehouseDistribution = [
  { name: '一级总库', amount: 682 },
  { name: '二级库-工段A', amount: 58 },
  { name: '二级库-工段B', amount: 42 },
  { name: '二级库-工段C', amount: 30 },
];
...
export const users: Record<UserRole, UserProfile> = {
  section_leader: { name: '刘工段长', role: 'section_leader', roleName: '工段长', avatar: '刘' },
  engineer: { name: '张工', role: 'engineer', roleName: '设备工程师', avatar: '张' },
  warehouse_keeper: { name: '王管', role: 'warehouse_keeper', roleName: '库管员', avatar: '王' },
  manager: { name: '陈总', role: 'manager', roleName: '管理层', avatar: '陈' },
};

// Helper to get part by code
export function getPartByCode(code: string): SparePart | undefined {
  return spareParts.find(p => p.code === code);
}

// Helper to get inventory for a warehouse
export function getInventoryByWarehouse(warehouse: string): (InventoryItem & { part: SparePart })[] {
  return inventory
    .filter(i => i.warehouse === warehouse)
    .map(i => ({ ...i, part: getPartByCode(i.partCode)! }))
    .filter(i => i.part);
}
